...
CREATE OR REPLACE PACKAGE BODY pkg_travel_display AS
    ...
    BEGIN
        ...
        DBMS_OUTPUT.PUT_LINE('Åšrednia ocena: ' || ROUND(p_record.srednia_ocena, 2));
    ...
    BEGIN
        v_result := 
            ...
            'Œrednia ocena: ' || ROUND(p_record.srednia_ocena, 2);
        RETURN v_result;
    ...
    BEGIN
        ...
        DBMS_OUTPUT.PUT_LINE(
            ...
            ', d³ugoœæ ' || p_min_duration || '-' || p_max_duration || ' dni' ||
...
CREATE OR REPLACE PACKAGE BODY pkg_reservation_management AS
   ...
   EXCEPTION
       ...
       WHEN ex_reservation_conflict THEN
           p_status := 'ERROR: U¿ytkownik ma ju¿ rezerwacjê w tym terminie';
           ROLLBACK;
